...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default="logs/MapC/2TD3_Lagrangian_04291633_5e-5", 
                        help='Directory with trained model')
    parser.add_argument('--check_point', type=int, default=21000, 
    ...
    log_dir = 'plot/MapC/data_trained_td3_lagrangian' 
    ...
    np.savez(log_dir+'/../spawn_points_td3_lagrangian', 
             center=np.array(sp), left=np.array(left_pt), right=np.array(right_pt))

    ############################
    # Load TD3-PIRL agent
    ############################
    agentOp = agentOptions(USE_GPU=not args.no_gpu)
    ...
    plt.savefig(f"{log_dir}/trajectories_td3_lagrangian.png", dpi=300, bbox_inches="tight")
    plt.show()
    ...
    if len(slip_angles_all) <= 5:
        plt.legend()
    plt.grid(True)
    plt.savefig(f"{log_dir}/slip_angles_td3_lagrangian.png", dpi=300, bbox_inches="tight")
    plt.show()
    ...
    if len(yaw_rates_all) <= 5:
        plt.legend()
    plt.grid(True)
    plt.savefig(f"{log_dir}/yaw_rates_td3_lagrangian.png", dpi=300, bbox_inches="tight")
    plt.show()

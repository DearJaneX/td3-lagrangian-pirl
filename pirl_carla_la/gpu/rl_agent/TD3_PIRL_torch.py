...
def agentOptions(
    ...
    agentOp = {
        ...
        'LOG_LAGRANGIAN_COMPONENTS' : LOG_LAGRANGIAN_COMPONENTS,
        }
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.steering_range = [-0.8, 0.8]  # CarEnv.step_S_pool range
        self.throttle_range = [0.6, 1.0]   # CarEnv.step_T_pool range
...
class TD3PIRLagent:
    def __init__(self, state_dim, action_dim, max_action, agentOp, pinnOp):
        ...
        self.q_value_term = 0          # -E[Q(s,π(s))]
        self.pde_residual_term = 0     # E[R_PDE^2]
        self.constraint_term = 0       # λ(E[R_PDE^2]-ε)
        self.lagrangian_total = 0      
    ...
    def clamp_log_lambda(self):

        with torch.no_grad():
            self.log_lambda.data.clamp_(max=self.agentOp['LOG_LAMBDA_MAX'])
    ...
    def compute_pde_residual(self, states, actions):
        ...
        dq1_dtau = grad_q1[:, 0].unsqueeze(1)  # derivative with respect to tau (first component)
    ...
    def get_constraint_decay_factor(self):
        ...
        if self.current_episode < start:
            ...
            progress = (self.current_episode - start) / (end - start)
            return 1.0 - progress
    ...
    def train_step(self, experience, is_episode_done):
        ...
        if self.total_it % self.agentOp['UPDATE_ACTOR_EVERY'] == 0:
            ...
            q_value = self.critic.Q1(state, actor_actions).mean()
            actor_loss_main = -q_value
            
            #(-E[Q(s,π(s))])
            self.q_value_term = -q_value.item()
            
            constraint_decay = self.get_constraint_decay_factor()
            
            if constraint_decay > 0:
                ...
                if self.pde_loss_avg == 0.0:
                    self.pde_loss_avg = pde_loss_actor.item()
                else:
                    self.pde_loss_avg = 0.95 * self.pde_loss_avg + 0.05 * pde_loss_actor.item()
                
                self.pde_residual_term = pde_loss_actor.item()
                ...
                self.constraint_term = lambda_value.item() * (pde_loss_actor.item() - epsilon)
                
                constraint_loss = lambda_value * (pde_loss_actor - epsilon) * constraint_decay
                actor_loss = actor_loss_main + constraint_loss
                
                self.lagrangian_total = self.q_value_term + self.constraint_term
                
                # Save PDE loss value for lambda update
                pde_loss_value = pde_loss_actor.item()
            else:
                actor_loss = actor_loss_main
                self.pde_residual_term = 0
                self.constraint_term = 0
                self.lagrangian_total = self.q_value_term
            ...
            # Update Lagrangian multiplier separately if constraint not decayed
            if constraint_decay > 0:
                # Get fresh lambda value
                lambda_value = torch.exp(self.log_lambda)
                
                # We want to maximize the dual function by minimizing -dual
                ...
                self.clamp_log_lambda()
                
                current_lambda = torch.exp(self.log_lambda).item()
                lambda_lr = self.agentOp['LAMBDA_LR'] / (1.0 + current_lambda / 10.0)
    ...
    def load_weights(self, ckpt_dir, ckpt_idx=None):
        ...
        # Load lambda if it exists in checkpoint
    ...
    def get_lambda(self):
        ...
        return torch.clamp(torch.exp(self.log_lambda), max=self.agentOp['LAMBDA_MAX']).item()

    def get_lagrangian_components(self):
        return {
            'q_value_term': self.q_value_term,
            'pde_residual_term': self.pde_residual_term, 
            'pde_residual_avg': self.pde_loss_avg,
            'constraint_term': self.constraint_term,
            'lagrangian_total': self.lagrangian_total,
            'epsilon': self.agentOp['PDE_EPSILON']
        }
...
def train(agent, env, trainOp):
    ...
    for episode in iterator:
        ...
        if trainOp['LOG_DIR']: 
            ...
            if agent.agentOp['LOG_LAGRANGIAN_COMPONENTS']:
                components = agent.get_lagrangian_components()
                summary_writer.add_scalar("Lagrangian/Q-Value Term", components['q_value_term'], episode)
                summary_writer.add_scalar("Lagrangian/PDE Residual", components['pde_residual_term'], episode)
                summary_writer.add_scalar("Lagrangian/PDE Residual Avg", components['pde_residual_avg'], episode)
                summary_writer.add_scalar("Lagrangian/Constraint Term", components['constraint_term'], episode)
                summary_writer.add_scalar("Lagrangian/Total", components['lagrangian_total'], episode)
                summary_writer.add_scalar("Lagrangian/Epsilon", components['epsilon'], episode)
            ...
            if trainOp['SAVE_AGENTS'] and episode % trainOp['SAVE_FREQ'] == 0:
                ...
                save_dict = {
                    ...
                    'lambda_value': agent.log_lambda.data
                }

"""
Verification script for TD3-PIRL safe drifting with Lagrangian method
"""
####################################
# general packages
import numpy as np
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default="logs/MapC/TD3_Lagrangian_04212156", 
    ...
    parser.add_argument('--use_lagrangian', action='store_true',
                        help='Whether the model used Lagrangian method')
    ...
    log_dir = 'plot/MapC/data_trained_td3_lagrangian' if args.use_lagrangian else 'plot/MapC/data_trained_td3'
    ...
    np.savez(log_dir+'/../spawn_points_td3_lagrangian' if args.use_lagrangian else log_dir+'/../spawn_points_td3', 
    ...
    # Display lambda value if using Lagrangian method
    ...
    # Plots
    #####################################
    method_name = "TD3-PIRL with Lagrangian Multiplier" if args.use_lagrangian else "TD3-PIRL"
    plt.title(f"Vehicle Trajectories with {method_name}")
    plt.xlabel("Y Position")
    plt.ylabel("X Position")
    plt.savefig(f"{log_dir}/trajectories_td3{'_lagrangian' if args.use_lagrangian else ''}.png", dpi=300, bbox_inches="tight")
    plt.show()
    ...
    if len(slip_angles_all) <= 5:
        plt.legend()
    plt.grid(True)
    plt.savefig(f"{log_dir}/slip_angles_td3{'_lagrangian' if args.use_lagrangian else ''}.png", dpi=300, bbox_inches="tight")
    plt.show()
    ...
    if len(yaw_rates_all) <= 5:
        plt.legend()
    plt.grid(True)
    plt.savefig(f"{log_dir}/yaw_rates_td3{'_lagrangian' if args.use_lagrangian else ''}.png", dpi=300, bbox_inches="tight")
    plt.show()

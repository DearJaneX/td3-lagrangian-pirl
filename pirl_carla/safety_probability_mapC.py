...
# Settings
###########################################################
log_dir = 'logs/MapC/5e-5/07282145'
...
def contour_plot(x, y, z, key = None, filename='plot'):
    #(x, y) = (e, psi)/ (e, vx)
    x, y = np.meshgrid(x, y)
    x = x.transpose()
    y = y.transpose()
    ...
    if not key:
        ...
        plt.title('Safety Probability Visualization (DQN-PIRL)')   
    elif key[0] == "e" and key[1] == 'psi':
        plt.xlabel('Lateral error $e$ [m]')
        plt.ylabel(r'Heading error $\psi$ [rad]')
        plt.title('Safety Probability vs Lateral and Heading Error (DQN-PIRL)')
    elif key[0] == "e" and key[1] == 'v':
        plt.xlabel('Lateral error $e$ [m]')
        plt.ylabel('Longitudinal velocity $v_x$ [m/s]')
        plt.title('Safety Probability vs Lateral Error and Velocity (DQN-PIRL)')
